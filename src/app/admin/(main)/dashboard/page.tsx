...
async function getDashboardData(viewMode: string, selectedYear: number) {
  ...
  (allApps || []).forEach((app) => {
    const sid = app.status_id as number;
    statusCount[sid] = (statusCount[sid] || 0) + 1;
  });

  const statusDistribution = Object.entries(STATUS_CONFIG)
    .map(([id, cfg]) => ({
      name: cfg.name,
      value: statusCount[Number(id)] || 0,
      color: cfg.color,
    }))
    .filter((s) => s.value > 0);

  const { data: allAppsDate } = await supabase
    .from("ncage_applications")
    .select("created_at");
  ...
  if (viewMode === "yearly") {
    ...
    (allAppsDate || []).forEach((app) => {
      const year = new Date(app.created_at).getFullYear();
      yearCount[year] = (yearCount[year] || 0) + 1;
    });
    for (let y = 2023; y <= currentYear; y++) {
      registrationTrend.push({ label: String(y), count: yearCount[y] || 0 });
    }
  } else {
    ...
    registrationTrend = MONTHS.map((label, i) => ({
      label,
      count: monthCount[i] || 0,
    }));
  }
  ...
  (provinceData || []).forEach((row) => {
    if (row.province && activeSet.has(row.ncage_application_id)) {
      provinceCount[row.province] = (provinceCount[row.province] || 0) + 1;
    }
  });

  const provinceDistribution = Object.entries(provinceCount).map(
    ([province, count]) => ({
      province,
      count,
    }),
  ...
  return {
    ...
    activeNcage: activeNcage || 0,
    inactiveNcage: inactiveNcage || 0,
    statusDistribution,
    registrationTrend,
